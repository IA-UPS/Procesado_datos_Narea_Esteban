...
set.seed(123456789)#semilla aleatoria para ver que los datos sean reproducibles
trControl <- trainControl(method = 'cv', number = 10)#parámetros de entranamiento
modelo <- train(clase ~ ., data = datos.train.lista$raw, method = "multinom", trControl = trControl, trace = FALSE)

# Predicciones en los datos de prueba
predicciones <- predict(modelo, newdata = datos.test.lista$raw)

# Matriz de confusión
library(caret)
confusionMatrix(data = predicciones, reference = datos.test.lista$raw$clase)
```

```{r}
#sensibilidad de la matriz de confusión 
